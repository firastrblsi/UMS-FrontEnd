...
import type { Program, TeachingModule, Curriculum } from '../types/university.types';
...
export default function UpdateCurriculumForm({ curriculum, onSuccess, onCancel }: UpdateCurriculumFormProps) {
  ...
  const { register, control, handleSubmit, formState: { errors }, watch } = useForm<CurriculumFormValues>({
    ...
    defaultValues: {
      ...
      yearNumber: curriculum.yearNumber,
      termNumber: curriculum.termNumber,
      modules: curriculum.modules?.map(m => ({
        teachingModuleId: m.teachingModuleId,
  ...
  const onSubmit = async (data: any) => {
    try {
      setIsLoading(true);
      await curriculumApi.updateCurriculum(curriculum.id, data);
